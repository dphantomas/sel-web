...
export default function CoursesGrid({ initialCourses, lang = 'es', hideHero = false }) {
  ...
  return (
    <div className={hideHero ? "w-full flex flex-col" : "min-h-screen bg-gray-50 flex flex-col"}>
      ...
      {selectedCourse && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center p-4 pt-16 md:pt-24 bg-black/60 backdrop-blur-sm animate-modal-overlay" 
          onClick={closeModal}
        >
          <div 
          ...
          >
            ...
            <div className="relative w-full flex-shrink-0 bg-purple-900 overflow-hidden" style={{ height: '240px', minHeight: '240px', display: 'block' }}>
              ...
              <button 
                onClick={closeModal} 
                className="absolute top-4 right-4 bg-black/30 hover:bg-black/50 backdrop-blur-md text-white p-2 rounded-full transition-all duration-200 shadow-sm z-50 cursor-pointer"
              >
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end flex-shrink-0 rounded-b-3xl">
              <button 
                onClick={closeModal}
                className="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-xl font-medium hover:from-purple-700 hover:to-purple-900 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
              >
                Volver al catálogo
              </button>
            </div>
