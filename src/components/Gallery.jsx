...
export default function Gallery({ lang = 'es', initialImages = [] }) {
  ...
  const handleError = (id) => setFailed((prev) => new Set(prev).add(id))
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {displayed.map((img, idx) => (
            <div
              key={img.id}
              className="gallery-item relative group rounded-xl overflow-hidden cursor-pointer aspect-square shadow-sm"
